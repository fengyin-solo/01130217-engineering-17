...
import { createChart, useManagedTimeout } from '@/composables/useChartManager'
...
const selectStage = (stage: Stage) => {
  ...
  // 延时初始化由 useManagedTimeout 托管，离开页面后回调不会再操作已销毁的图表
  useManagedTimeout(() => initTrendChart(), 100)
}
...
const initDurationChart = () => {
  ...
  createChart(durationChart.value, {
...
// 趋势图位于懒渲染的 tab-pane 中：首次切换到“数据趋势”时容器才挂载，
// 此时再初始化；重复切换因 createChart 幂等而安全
watch(activeTab, tab => {
  if (tab === 'trend') useManagedTimeout(() => initTrendChart(), 0)
})
